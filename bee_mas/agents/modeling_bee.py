...
class ModelingBee(SmartBeeAgent):
    ...
    def _build_design_requirement(self, task: TaskAnnouncement) -> str:
        """构建设计需求摘要，满足Dify API的字符限制"""
        # 获取任务输入参数
        ...
        if dependency_results:
            for dep_id, dep_data in dependency_results.items():
                ...
                if "parameters" in dep_data["result"] and dep_data["result"]["parameters"]:
                    print(f"      找到参数: {len(dep_data['result']['parameters'])} 个")
                    # 取所有关键参数
                    param_list = []
                    for k, v in dep_data["result"]["parameters"].items():
    ...
    def _call_dify_3d_modeling(self, task: TaskAnnouncement) -> Dict[str, Any]:
        ...
        # 添加映射后的design_requirement参数
        design_requirement = self._build_design_requirement(task)
        
        # 构建增强的输入参数，包含依赖任务的输出结果
        enhanced_inputs = {
            "design_requirement": design_requirement,  # 映射后的参数
        ...
        try:
            ...
            print(f"   📝 设计需求: {design_requirement[:100]}...")
